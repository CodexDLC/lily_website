...
def debug_wizard():
    ...
    with patch("features.booking.views.public.wizard.get_cart", return_value=mock_cart), \
        ...
        mock_provider.get_public_services.return_value = [{
            'id': 1, 'slug': 'test-service', 'title': 'EXPECTED_SERVICE_TITLE', 
            'duration': 60, 'price': '100.00', 'category_slug': 'cat', 'category_name': 'Category'
        }]
...
def debug_commit():
    ...
    with patch("features.booking.selector.engine.get_booking_engine_gateway") as mock_engine_func, \
