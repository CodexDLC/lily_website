...
for _mod_name in ["django_ratelimit", "django_ratelimit.decorators"]:
    if _mod_name not in sys.modules:
        ...
        _fake_rl.ratelimit = lambda *a, **kw: lambda f: f  # type: ignore[attr-defined]
...
def mock_site_settings_redis():
    ...
    if "django_redis" not in sys.modules:
        ...
        fake_django_redis.get_redis_connection = MagicMock(return_value=MagicMock())  # type: ignore[attr-defined]
