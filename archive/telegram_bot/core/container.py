...
class BotContainer(BaseBotContainer):
    ...
    async def process_redis_message(self, message: dict[str, Any]) -> None:
        ...
        """
        if self.i18n:
            locale = str(message.get("locale") or message.get("lang") or self.i18n.core.default_locale or "de")
            with self.i18n.use_context(locale=locale, data={"message_data": message}):
