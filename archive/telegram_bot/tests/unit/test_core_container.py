...
class TestBotContainer:
    ...
    @pytest.mark.asyncio
    async def test_process_redis_message_uses_i18n_context(self, settings, mock_redis):
        container = BotContainer(settings, mock_redis)
        container.redis_dispatcher = MagicMock()
        container.redis_dispatcher.process_message = AsyncMock()

        mock_i18n = MagicMock()
        mock_i18n.core.default_locale = "de"
        mock_context = MagicMock()
        mock_i18n.use_context.return_value = mock_context
        container.i18n = mock_i18n

        message = {"type": "new_appointment", "locale": "ru"}
        await container.process_redis_message(message)

        mock_i18n.use_context.assert_called_once_with(locale="ru", data={"message_data": message})
        container.redis_dispatcher.process_message.assert_awaited_once_with(message)
