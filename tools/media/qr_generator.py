...
class QRGeneratorApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.fg_var = tk.StringVar(value=self.fg_color)
        self.fg_var.trace_add("write", lambda *a: self._on_hex_change("fg"))
        # Using tk.Entry instead of ttk.Entry for better clipboard behavior
        self.fg_entry = tk.Entry(fg_frame, textvariable=self.fg_var, width=10)
        self.fg_entry.pack(side=tk.RIGHT, padx=5)
        self._add_context_menu(self.fg_entry)
        ...
        self.bg_var = tk.StringVar(value=self.bg_color)
        self.bg_var.trace_add("write", lambda *a: self._on_hex_change("bg"))
        self.bg_entry = tk.Entry(bg_frame, textvariable=self.bg_var, width=10)
        self.bg_entry.pack(side=tk.RIGHT, padx=5)
        self._add_context_menu(self.bg_entry)
    ...
    def _on_hex_change(self, target: str) -> None:
        """Called when hex entry is modified."""
        var = self.fg_var if target == "fg" else self.bg_var
        ...
        if len(val) == 7 and val.startswith("#"):
            try:
                ...
                if target == "fg":
                    ...
                    self.bg_color = val
                btn.configure(bg=val)
            except tk.TclError:
                pass

    def _add_context_menu(self, widget: tk.Entry) -> None:
        """Adds explicit right-click context menu and clipboard support."""
        ...
        def do_paste():
            try:
                ...
                # If there's a selection, replace it. Otherwise clear and paste.
        ...
        # Binding to all possible Win/Linux right-click buttons
        widget.bind("<Button-3>", show_menu)
        widget.bind("<Button-2>", show_menu)

        # Explicitly overriding standard key shortcuts to use our robust methods
        widget.bind("<Control-v>", lambda e: do_paste() or "break")
        widget.bind("<Control-V>", lambda e: do_paste() or "break")
        widget.bind("<Control-c>", lambda e: do_copy() or "break")
        widget.bind("<Control-C>", lambda e: do_copy() or "break")
