...
def mock_settings():
    ...
    settings.SENDGRID_API_KEY = "sg_key"  # pragma: allowlist secret
    return settings
...
async def test_init_common_dependencies(mock_settings):
    ...
    with (
        patch("src.workers.core.base_module.dependencies.from_url") as mock_from_url,
...
async def test_close_common_dependencies(mock_settings):
    ...
    ctx = {"internal_api": mock_api, "redis_client": mock_redis}
