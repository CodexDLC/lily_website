...
from src.workers.notification_worker.tasks.message_tasks import send_appointment_notification, send_message_task
...
async def test_send_message_task_success():
    ...
    with patch(
        "src.workers.notification_worker.tasks.message_tasks._send_status_update", new_callable=AsyncMock
    ) as mock_status:
...
async def test_send_appointment_notification_full_flow():
    ...
    ctx["arq_service"].enqueue_job.assert_any_call(
        ...
        appointment_id=123,
        media_url="http://logo",
        variables={"1": "Anna", "2": "20.10.2024", "3": "10:00", "4": "123"},
    )
...
async def test_send_appointment_notification_date_parsing_fallback():
    ...
    appointment_data = {"client_phone": "+49123", "datetime": "2024-10-20 (unexpected format)"}
    ...
    for call in ctx["arq_service"].enqueue_job.call_args_list:
        if call.args[0] == "send_message_task":
            ...
            assert variables["2"] == "2024-10-20"  # fallback to split[0]
    assert found
