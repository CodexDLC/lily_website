...
from src.workers.notification_worker.tasks.email_tasks import _mailbox_headers, send_email_task
...
async def test_send_email_task_success():
    ...
    with patch(
        "src.workers.notification_worker.tasks.email_tasks._send_status_update", new_callable=AsyncMock
    ) as mock_status:
