...
class NotificationsUI:
    ...
    Единственная точка входа для формирования интерфейса (текст + кнопки).
    """
    ...
    def render_main(self, text: str, appointment_id: int, topic_id: int | None = None) -> ViewResultDTO:
        """Отрисовка основного экрана управления (Подтвердить/Отклонить)."""
    ...
    def render_post_action(self, text: str, appointment_id: int, topic_id: int | None = None) -> ViewResultDTO:
        """Отрисовка экрана после выполнения действия (кнопка Удалить)."""
    ...
    def render_email_data(
        ...
        """Формирование данных для Email через форматтер."""
