...
class StartOrchestrator(BaseBotOrchestrator):
    ...
    async def render(self, user_id: int, user_name: str = "User") -> UnifiedViewDTO:
        ...
        return UnifiedViewDTO(
            ...
            content=None,
            clean_history=True,
            chat_id=user_id,  # Обязательно для первого сообщения
            session_key=user_id,  # Обязательно для Redis
        )
    ...
    async def handle_entry(self, user_id: int, payload: Any = None) -> UnifiedViewDTO:
        ...
        return await self.render(user_id=user_id, user_name=user_name)
