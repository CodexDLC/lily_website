...
class SiteSettingsManager:
    ...
    def save_to_redis(cls, site_settings_instance: Any):
        ...
        """
        log.debug("Redis: SiteSettingsManager | Action: SaveToRedis")
        try:
            ...
            log.info(f"Redis: SiteSettingsManager | Action: Success | fields_count={len(sanitized_dict)}")
        except Exception as e:
            log.error(f"Redis: SiteSettingsManager | Action: SaveFailed | error={e}")
    ...
    def load_from_redis(cls) -> dict:
        ...
        """
        log.debug("Redis: SiteSettingsManager | Action: LoadFromRedis")
        try:
            redis_client = cls.get_redis_client()
            cached_data = redis_client.hgetall(REDIS_SITE_SETTINGS_KEY)

            if cached_data:
                log.debug(f"Redis: SiteSettingsManager | Action: CacheHit | fields_count={len(cached_data)}")
                ...
                log.warning("Redis: SiteSettingsManager | Action: CacheMiss | status=FetchingFromDB")
            ...
            log.error(f"Redis: SiteSettingsManager | Action: LoadFailed | error={e}")
            # Fallback to DB if Redis fails
            from features.system.models.site_settings import SiteSettings

            return SiteSettings.load().to_dict()
