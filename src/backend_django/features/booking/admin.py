...
class BookingSettingsAdmin(ModelAdmin):
    ...
    fieldsets = (
        (_("General Settings"), {"fields": ("default_step_minutes", "default_buffer_between_minutes", "default_min_advance_minutes", "default_max_advance_days")}),
        (_("Notifications"), {"fields": ("reminder_hours_before", "admin_email_for_notifications", "telegram_notification_channel_id")}),
        (_("Cancellation Policy"), {"fields": ("cancellation_window_hours",)}),
    )
...
class MasterAdmin(ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        return mark_safe(f'<span class="px-2 py-1 rounded-md text-xs font-medium {color_classes}">{obj.get_status_display()}</span>')
...
class ClientAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Socials"), {"fields": ("telegram", "instagram")}),
        (_("Status"), {"fields": ("status", "notes", "consent_marketing")}),
        (_("System"), {"fields": ("created_at", "updated_at", "user")}),
    )
    ...
    def status_badge(self, obj):
        ...
        return mark_safe(f'<span class="px-2 py-1 rounded-md text-xs font-medium {colors.get(obj.status, "bg-gray-500/20")}">{obj.get_status_display()}</span>')

@admin.register(Appointment)
class AppointmentAdmin(ModelAdmin):
    list_display = ("id", "client_link", "master_link", "service_link", "datetime_start", "status_badge", "price", "source")
    ...
    def client_link(self, obj): return obj.client
    @admin.display(description=_("Master"))
    def master_link(self, obj): return obj.master
    @admin.display(description=_("Service"))
    def service_link(self, obj): return obj.service
